...
use tw_crypto::crypto_hmac::hmac_sha256;
...
fn test_hmac_sha256() {
    ...
    let data = hmac_sha256(&password, &salt, 1, 20);
    assert_eq!(
        hex::encode(&data, false),
        "120fb6cffcf8b32c43e7225256c4f837a86548c9"
    );

    let data = hmac_sha256(&password, &salt, 4096, 20);
    assert_eq!(
        hex::encode(&data, false),
    ...
    let data = hmac_sha256(&password, &salt2, 100, 32);
    assert_eq!(
        hex::encode(&data, false),
