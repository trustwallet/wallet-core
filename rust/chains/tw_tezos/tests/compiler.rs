...
fn test_compile_with_signatures() {
    ...
    let pre_image_hash_data =
        TezosCompiler::preimage_hashes(&TestCoinContext::default(), input.clone());
    assert_eq!(pre_image_hash_data.error, CommonProto::SigningError::OK);
    assert_eq!(
        hex::encode(&pre_image_hash_data.data_hash, false),
    ...
    assert_eq!(
        output.error,
        CommonProto::SigningError::Error_signatures_count
    );
}
