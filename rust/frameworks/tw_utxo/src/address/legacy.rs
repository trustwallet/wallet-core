...
pub type BitcoinBase58Address =
    Base58Address<BITCOIN_ADDRESS_SIZE, BITCOIN_ADDRESS_CHECKSUM_SIZE, Sha256d>;

pub type LegacyAddress = PrefixedBase58Address<Sha256Ripemd, Sha256d>;
...
pub struct PrefixedBase58Address<PublicKeyHasher, ChecksumHasher> {
    inner: Base58Address<BITCOIN_ADDRESS_SIZE, BITCOIN_ADDRESS_CHECKSUM_SIZE, ChecksumHasher>,
    _phantom: PhantomData<PublicKeyHasher>,
}

impl<PublicKeyHasher, ChecksumHasher> PrefixedBase58Address<PublicKeyHasher, ChecksumHasher>
where
    ...
    ChecksumHasher: StaticHasher,
{
    pub fn new(prefix: u8, data: &[u8]) -> AddressResult<Self> {
        ...
        Base58Address::new(&bytes, Alphabet::Bitcoin).map(|inner| PrefixedBase58Address {
            inner,
    ...
    pub fn p2pkh_with_public_key(
        ...
        PrefixedBase58Address::new(p2pkh_prefix, &public_key_hash)
    }
    ...
    pub fn p2pkh_with_coin_and_prefix(
        ...
        PrefixedBase58Address::p2pkh_with_public_key(p2pkh_prefix, ecdsa_public_key)
    }
    ...
    pub fn p2sh_with_prefix_byte(redeem_script: &Script, p2sh_prefix: u8) -> AddressResult<Self> {
        ...
        PrefixedBase58Address::new(p2sh_prefix, &script_hash)
    }
    ...
    pub fn from_str_with_coin_and_prefix(
        ...
        PrefixedBase58Address::from_str_checked(s, base58_prefix.p2pkh, base58_prefix.p2sh)
    }

    pub fn from_str_checked(s: &str, p2pkh_prefix: u8, p2sh_prefix: u8) -> AddressResult<Self> {
        let addr = PrefixedBase58Address::from_str(s)?;
...
impl<PublicKeyHasher, ChecksumHasher> FromStr
    for PrefixedBase58Address<PublicKeyHasher, ChecksumHasher>
where
    PublicKeyHasher: StaticHasher,
    ChecksumHasher: StaticHasher,
{
