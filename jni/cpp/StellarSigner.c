...
#include "TWJNI.h"
#include "StellarSigner.h"

jobject JNICALL Java_wallet_core_jni_StellarSigner_sign(JNIEnv *env, jclass thisClass, jobject input) {
    ...
    jbyteArray resultData = TWDataJByteArray(TWStellarSignerSign(inputData), env);
    jclass resultClass = (*env)->FindClass(env, "wallet/core/jni/proto/Stellar$SigningOutput");
    jmethodID parseFromMethodID = (*env)->GetStaticMethodID(env, resultClass, "parseFrom", "([B)Lwallet/core/jni/proto/Stellar$SigningOutput;");
