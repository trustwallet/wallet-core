...
Wallet core follows the [LLVM Coding Standards](http://llvm.org/docs/CodingStandards.html) for C++. We use `clang-format` to ensure a consistent code sytle. Please run `git clang-format` on your code before you submit a PR. **Do not** reformat files that you didn't modify. You can also install a clang-format extension for your IDE.
