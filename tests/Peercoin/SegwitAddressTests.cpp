...
bool peercoin_case_insensitive_equal(const std::string& s1, const std::string& s2) {
    size_t i = 0;
...
TEST(PeercoinSegwitAddress, ValidChecksum) {
    for (auto i = 0; i < sizeof(valid_checksum) / sizeof(valid_checksum[0]); ++i) {
        ...
        ASSERT_TRUE(peercoin_case_insensitive_equal(recode, valid_checksum[i]));
    }
}

TEST(PeercoinSegwitAddress, InvalidChecksum) {
...
TEST(PeercoinSegwitAddress, ValidAddress) {
    for (auto i = 0; i < sizeof(valid_address) / sizeof(valid_address[0]); ++i) {
        ...
        ASSERT_TRUE(peercoin_case_insensitive_equal(valid_address[i].address, recode));
    }
}

TEST(PeercoinSegwitAddress, InvalidAddress) {
...
TEST(PeercoinSegwitAddress, InvalidAddressEncoding) {
...
TEST(PeercoinSegwitAddress, LegacyAddress) {
