...
TEST(HDWallet, privateKeyFromDGRV) {
    ...
    auto publicKey = privateKey->getPublicKey(TWPublicKeyTypeSECP256k1);
    auto address = Bitcoin::Address(publicKey, TWCoinTypeConfigurationGetP2PKHPrefix(TWCoinTypeDogecoin));
...
TEST(HDWallet, privateKeyFromXPRVForDGB) {
    ...
    auto publicKey = privateKey->getPublicKey(TWPublicKeyTypeSECP256k1);
    auto address = Bitcoin::Address(publicKey, TWCoinTypeConfigurationGetP2PKHPrefix(TWCoinTypeDigiByte));
