...
TEST(HDWallet, createFromMnemonicUNCHECKED) {
    {
        HDWallet wallet = HDWallet(mnemonic1, passphrase, parse_hex("ba5821e8c356c05ba5f025d9532fe0f21f65d594"));
        EXPECT_EQ(wallet.getMnemonic(), mnemonic1);
        EXPECT_EQ(wallet.getPassphrase(), passphrase);
        EXPECT_EQ(hex(wallet.getEntropy()), "ba5821e8c356c05ba5f025d9532fe0f21f65d594");
        EXPECT_EQ(hex(wallet.getSeed()), "143cd5fc27ae46eb423efebc41610473f5e24a80f2ca2e2fa7bf167e537f58f4c68310ae487fce82e25bad29bab2530cf77fd724a5ebfc05a45872773d7ee2d6");
    }
    {   // Set a mnemonic and non-matching entropy -- possible, but dangerous
        HDWallet wallet = HDWallet(mnemonic1, passphrase, parse_hex("000102030405060708090a0b0c0d0e0f10111213"));
        EXPECT_EQ(wallet.getMnemonic(), mnemonic1);
        EXPECT_EQ(wallet.getPassphrase(), passphrase);
        EXPECT_EQ(hex(wallet.getEntropy()), "000102030405060708090a0b0c0d0e0f10111213");
        EXPECT_EQ(hex(wallet.getSeed()), "143cd5fc27ae46eb423efebc41610473f5e24a80f2ca2e2fa7bf167e537f58f4c68310ae487fce82e25bad29bab2530cf77fd724a5ebfc05a45872773d7ee2d6");
    }
    {   // Set invalid mnemonic -- possible, but dangerous
        HDWallet wallet = HDWallet("THIS IS INVALID MNEMONIC", passphrase, parse_hex("000102030405060708090a0b0c0d0e0f10111213"));
        EXPECT_EQ(wallet.getMnemonic(), "THIS IS INVALID MNEMONIC");
        EXPECT_EQ(wallet.getPassphrase(), passphrase);
        EXPECT_EQ(hex(wallet.getEntropy()), "000102030405060708090a0b0c0d0e0f10111213");
        EXPECT_EQ(hex(wallet.getSeed()), "c7b772c54e02ce971da1e8b5283e675b13c41f40bac5d51016c6440c71b36eeab9c5317d269fadfc65ad5395e4b95a0d19a240ed6206c7ba874129ee471f0bc1");
    }
    EXPECT_EXCEPTION(HDWallet("", passphrase, parse_hex("ba5821e8c356c05ba5f025d9532fe0f21f65d594")), "Invalid mnemonic");
    EXPECT_EXCEPTION(HDWallet(mnemonic1, passphrase, parse_hex("00010203040506070809")), "Invalid entropy");
}
...
TEST(HDWallet, SpanishXpubBtcEth) {
    // using Unchecked wallet generation with Spanish mnemonic
    const Data entropy = parse_hex("81d8185b64f0371457198b43c22b6c43");
    HDWallet wallet = HDWallet("llanto radical atraer riesgo actuar masa fondo cielo dieta archivo sonrisa mamut", "", entropy);
