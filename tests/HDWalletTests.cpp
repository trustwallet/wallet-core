...
TEST(HDWallet, entropyLength_createFromMnemonic) {
    ...
    {   // 24 words
        ...
        EXPECT_EQ(wallet.getEntropy().size(), 33);
        EXPECT_EQ(hex(wallet.getEntropy()), "a73a3732edebbb49f5fdfe68c7b5c0f6e9de3a1d5760faa8c771e384bf4229b6d9");
    }
    {   // 24 words, from https://github.com/trezor/python-mnemonic/blob/master/vectors.json
        ...
        EXPECT_EQ(wallet.getEntropy().size(), 33);
        EXPECT_EQ(hex(wallet.getEntropy()), "8080808080808080808080808080808080808080808080808080808080808080bd");
    }
}
...
TEST(HDWallet, Bip39Vectors) {
    ...
    for (const auto& v: vectors) {
        ...
        { // from mnemonic
            ...
            EXPECT_EQ(wallet.getPassphrase(), passphrase);
            if (entropy.size() == 64) {
                // for 24-word mnemonic (entropy=32 size) our entropy calculation is wrong, truncate
                EXPECT_EQ(hex(wallet.getEntropy()).substr(0, 64), entropy);
            } else {
