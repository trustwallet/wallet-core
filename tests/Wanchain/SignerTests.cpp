...
TEST(WanchainSigner, Sign) {
    ...
    auto sig = signer.sign(key, transaction);
    auto encoded = signer.encode(transaction, sig);
    ...
    ASSERT_EQ(sig.v, 37);
    ASSERT_EQ(sig.r, uint256_t("81387416166961732074688826827458851755389597777750181844865627349674117325426"));
    ASSERT_EQ(sig.s, uint256_t("57615878674481745412864351256912693004505281517367788339541976444424345429095"));
}
