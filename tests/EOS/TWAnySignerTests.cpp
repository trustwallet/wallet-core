...
TEST(TWAnySignerEOS, Sign) {
...

    {
        ...
        EXPECT_EQ(output.error().code(), Common::Proto::No_error);
        EXPECT_TRUE(output.error().text().empty());
    ...
    input.set_private_key_type(Proto::KeyType::LEGACY);
    {
        Proto::SigningOutput output;
        ANY_SIGN(input, TWCoinTypeEOS);
        EXPECT_EQ(output.error().code(), Common::Proto::Error_general);
        EXPECT_FALSE(output.error().text().empty());
