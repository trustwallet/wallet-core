...
TEST(CardanoTransaction, getPolicyIDs) {
    ...
    EXPECT_TRUE(tb.getPolicyIds().count(policyId1) > 0);
    EXPECT_TRUE(tb.getPolicyIds().count(policyId2) > 0);

    EXPECT_EQ(tb.getByPolicyId(policyId1).size(), 1);
    EXPECT_EQ(tb.getByPolicyId(policyId2).size(), 2);
}
