...
TEST(PrivateKeySecp256k1Tests, CreateInvalid) {
    uint8_t bytes[] = {0xde, 0xad, 0xbe, 0xef};
    auto data = WRAPD(TWDataCreateWithBytes(bytes, 4));
    auto privateKey = WRAP(TWPrivateKeySecp256k1, TWPrivateKeySecp256k1CreateWithData(data.get()));

    ASSERT_EQ(privateKey.get(), nullptr);
}

TEST(PrivateKeySecp256k1Tests, IsValid) {
    ...
    ASSERT_TRUE(TWPrivateKeySecp256k1IsValid(data.get()));
}

TEST(PrivateKeySecp256k1Tests, PublicKey) {
    uint8_t bytes[] = {0xaf, 0xee, 0xfc, 0xa7, 0x4d, 0x9a, 0x32, 0x5c, 0xf1, 0xd6, 0xb6, 0x91, 0x1d, 0x61, 0xa6, 0x5c, 0x32, 0xaf, 0xa8, 0xe0, 0x2b, 0xd5, 0xe7, 0x8e, 0x2e, 0x4a, 0xc2, 0x91, 0x0b, 0xab, 0x45, 0xf5};
    auto data = WRAPD(TWDataCreateWithBytes(bytes, 32));
    auto privateKey = TWPrivateKeySecp256k1CreateWithData(data.get());
    auto publicKey = TWPrivateKeySecp256k1GetPublicKey(privateKey, false);
...
TEST(PrivateKeySecp256k1Tests, ClearMemory) {
    uint8_t bytes[] = {0xaf, 0xee, 0xfc, 0xa7, 0x4d, 0x9a, 0x32, 0x5c, 0xf1, 0xd6, 0xb6, 0x91, 0x1d, 0x61, 0xa6, 0x5c, 0x32, 0xaf, 0xa8, 0xe0, 0x2b, 0xd5, 0xe7, 0x8e, 0x2e, 0x4a, 0xc2, 0x91, 0x0b, 0xab, 0x45, 0xf5};
    auto data = WRAPD(TWDataCreateWithBytes(bytes, 32));
    auto privateKey = TWPrivateKeySecp256k1CreateWithData(data.get());
    TWPrivateKeySecp256k1Delete(privateKey);

    uint8_t *ptr = reinterpret_cast<uint8_t *>(privateKey);
    for (auto i = 0; i < TWPrivateKeySecp256k1Size; i += 1) {
        ASSERT_EQ(ptr[i], 0);
    }
}

TEST(PrivateKeySecp256k1Tests, Sign) {
    uint8_t bytes[] = {0xaf, 0xee, 0xfc, 0xa7, 0x4d, 0x9a, 0x32, 0x5c, 0xf1, 0xd6, 0xb6, 0x91, 0x1d, 0x61, 0xa6, 0x5c, 0x32, 0xaf, 0xa8, 0xe0, 0x2b, 0xd5, 0xe7, 0x8e, 0x2e, 0x4a, 0xc2, 0x91, 0x0b, 0xab, 0x45, 0xf5};
    auto keyData = WRAPD(TWDataCreateWithBytes(bytes, 32));
    auto privateKey = WRAP(TWPrivateKeySecp256k1, TWPrivateKeySecp256k1CreateWithData(keyData.get()));

    auto message = "hello";
    auto data = WRAPD(TWDataCreateWithBytes((uint8_t *)message, strlen(message)));
    auto hash = WRAPD(TWHashKeccak256(data.get()));

    auto actual = WRAPD(TWPrivateKeySecp256k1Sign(privateKey.get(), hash.get()));
