...
TEST(SteemOperation, Serialization) {
	Data buf;
	TransferOperation("alice", "bob", Asset(90000, Asset::decimals, Steem::MainNetAssetSymbol), "Eva").serialize(buf);
	ASSERT_EQ(
		hex(buf),
	...
	buf.clear();
	TransferOperation("Ram", "Sita", Asset(1000, Asset::decimals, Steem::MainNetAssetSymbol), "Lakshman").serialize(buf);
	ASSERT_EQ(
		hex(buf),
