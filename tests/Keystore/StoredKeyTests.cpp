...
TEST(StoredKey, LoadPBKDF2Key) {
    ...
    ASSERT_TRUE(std::holds_alternative<PBKDF2Parameters>(payload.kdfParams));
    EXPECT_EQ(std::get<PBKDF2Parameters>(payload.kdfParams).desiredKeyLength, 32);
    EXPECT_EQ(std::get<PBKDF2Parameters>(payload.kdfParams).iterations, 262144);
    EXPECT_EQ(hex(std::get<PBKDF2Parameters>(payload.kdfParams).salt), "ae3cd4e7013836a3df6bd7241b12db061dbe2c6785853cce422d148a624ce0bd");
...
TEST(StoredKey, ReadWallet) {
    ...
    ASSERT_TRUE(std::holds_alternative<ScryptParameters>(header.kdfParams));
    EXPECT_EQ(std::get<ScryptParameters>(header.kdfParams).desiredKeyLength, 32);
    EXPECT_EQ(std::get<ScryptParameters>(header.kdfParams).n, 262144);
    EXPECT_EQ(std::get<ScryptParameters>(header.kdfParams).p, 8);
    EXPECT_EQ(std::get<ScryptParameters>(header.kdfParams).r, 1);
    EXPECT_EQ(hex(std::get<ScryptParameters>(header.kdfParams).salt), "ab0c7876052600dd703518d6fc3fe8984592145b591fc8fb5c6d43190334ba19");
}
