...
bool performAnySign(TWCoinType coinType, const string& transaction, const string& privateKeyHex, TW::Any::Proto::SigningOutput& out) {
    ...
    si.set_coin_type((uint32_t)coinType);
    si.set_transaction(transaction);
    si.set_private_key(privateKeyHex);
    string siSerialized;
    if (!si.SerializeToString(&siSerialized)) return false;
    ...
    if(!so.ParseFromString(outStr)) return false;
    out = so;
    return true;
}
...
TEST(TWAnySigner, SignEthereum) {
    ...
    TW::Any::Proto::SigningOutput so;
    ASSERT_TRUE(performAnySign(coinType, transaction, privateKeyHex, so));

    ASSERT_TRUE(so.success());
    ASSERT_FALSE(so.has_error());
    // OK, get output
    EXPECT_EQ(
