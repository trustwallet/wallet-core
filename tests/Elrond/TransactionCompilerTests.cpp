...
TEST(ElrondCompiler, CompileGenericActionWithSignatures) {
    ...
    auto coin = TWCoinTypeElrond;
    auto input = TW::Elrond::Proto::SigningInput();
    ...
    { // Negative: inconsistent signatures & publicKeys
        ...
        Elrond::Proto::SigningOutput output;
        ASSERT_TRUE(output.ParseFromArray(outputData.data(), (int)outputData.size()));
...
TEST(ElrondCompiler, CompileEGLDTransferWithSignatures) {
...
TEST(ElrondCompiler, CompileESDTTransferWithSignatures) {
    // txHash d399477c5d2784d55521fadb2692d447e3459c6a006b1720a3bd513c68dc6848 on testnet
    /// Step 1: Prepare transaction input (protobuf)
    auto coin = TWCoinTypeElrond;
    auto input = TW::Elrond::Proto::SigningInput();
    input.mutable_esdt_transfer()->mutable_accounts()->set_sender_nonce(2487);
    ...
    input.mutable_esdt_transfer()->set_amount("1");
    input.mutable_esdt_transfer()->set_token_identifier("MBONDTEST-4ce053");
    input.mutable_esdt_transfer()->set_data("foo");
    input.set_gas_price(1000000000);
    input.set_gas_limit(11100000);
    input.set_chain_id("T");
    auto inputString = input.SerializeAsString();
    ...
    auto privateKey = PrivateKey(parse_hex(ALICE_SEED_HEX));
    const PublicKey publicKey = privateKey.getPublicKey(TWPublicKeyTypeED25519);
    Data signature = parse_hex("747c8ed465f12379592cc141895ef6972f8934c2305e042a50f83d02025b8c9ed73fc19739af5686dd4733023b5b967fa6e7f939295fa9a56f4b241438eeec0f");
    // Verify signature (pubkey & hash & signature)
    EXPECT_TRUE(publicKey.verify(signature, preImageHash));
    /// Step 3: Compile transaction info
    auto outputData =
