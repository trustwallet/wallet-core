...
TEST(TWNervos, AddressFromPublicKey) {
    auto privateKey =
        WRAP(TWPrivateKey,
    ...
    assertStringsEqual(address2String, "ckb1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqw"
                                       "yk5x9erg8furras980hksatlslfaktks7epf25");

    ASSERT_TRUE(TWAnyAddressEqual(address.get(), address2.get()));
}

TEST(TWNervos, AddressFromWallet) {
    auto wallet = WRAP(
        TWHDWallet,
