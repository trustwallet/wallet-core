...
TEST(ZilliqaSignature, Signing) {
    ...
    auto pubKey = WRAP(TWPublicKey, TWPrivateKeyGetPublicKeyZilliqaSchnorr(privateKey.get()));

    auto message = "hello schnorr";
    auto messageData = WRAPD(TWDataCreateWithBytes((uint8_t *)message, strnlen(message, 13)));
    auto signatureData = WRAPD(TWPrivateKeySign(privateKey.get(), messageData.get()));
    auto signature = data(TWDataBytes(signatureData.get()), TWDataSize(signatureData.get()));

    ASSERT_TRUE(TWPublicKeyVerify(pubKey.get(), signatureData.get(), messageData.get()));
