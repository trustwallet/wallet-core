...
TEST(NULSSigner, SignWithFeePayer) {
    auto privateKey =
...
TEST(NULSSigner, SignTokenWithFeePayer) {
    auto privateKey =
        parse_hex("0x48c91cd24a27a1cdc791022ff39316444229db1c466b3b1841b40c919dee3002");
...
TEST(NULSSigner, InsufficientBalance) {
    auto privateKey =
        parse_hex("0x48c91cd24a27a1cdc791022ff39316444229db1c466b3b1841b40c919dee3002");
    auto feePayerPrivateKey =
        parse_hex("0x9401fd554cb700777e57b05338f9ff47597add8b23ce9f1c8e041e9b4e2116b6");
    auto amount = store(uint256_t(10000000));
    auto balance = store(uint256_t(100));
    auto feePayerBalance = store(uint256_t(1000000));
    std::string nonce = "0000000000000000";
    Proto::SigningInput input;
    ...
    EXPECT_EQ(output.error_message(), "User account balance not sufficient");
}

TEST(NULSSigner, InsufficientBalanceWithFeePayer) {
    auto privateKey =
        parse_hex("0x48c91cd24a27a1cdc791022ff39316444229db1c466b3b1841b40c919dee3002");
    auto feePayerPrivateKey =
        parse_hex("0x9401fd554cb700777e57b05338f9ff47597add8b23ce9f1c8e041e9b4e2116b6");
    auto amount = store(uint256_t(10000000));
    auto balance = store(uint256_t(100000000));
    auto feePayerBalance = store(uint256_t(100));
    std::string nonce = "0000000000000000";
    Proto::SigningInput input;
    ...
    EXPECT_EQ(output.error(), Common::Proto::Error_general);
    EXPECT_EQ(output.error_message(), "fee payer balance not sufficient");
}

TEST(NULSSigner, TokenInsufficientBalance) {
    auto privateKey =
        parse_hex("0x9ce21dad67e0f0af2599b41b515a7f7018059418bab892a7b68f283d489abc4b");
    auto amount = store(uint256_t(10000000));
    auto balance = store(uint256_t(100000000));
