...
TEST(TWZcashTransaction, SendToTexAddress) {
    // tx on mainnet
    // https://blockchair.com/zcash/transaction/e5f4d0c28f96c80caace4052aed4645a78badedc00cd3b071322d63c264b2885

    const auto myAddress = "t1a6Do9CH4umduHKtoufDDMQMypd7VYLqhH";
    const auto myPrivateKey = parse_hex("cfa0c168cce0d041119b216f60aed343b93fde0a8736ea0bcd6254cfef0a3c79");

    auto input = Bitcoin::Proto::SigningInput();
    input.set_coin_type(TWCoinTypeZcash);
    input.set_hash_type(TWBitcoinSigHashTypeAll);
    input.set_zip_0317(true);
    input.set_to_address("tex1auz6gx89x2wcku6gswdvaz2nf9x3seex6px6v0");
    input.set_change_address(myAddress);
    input.set_amount(200'000);
    input.add_private_key(myPrivateKey.data(), myPrivateKey.size());

    auto txHash = parse_hex("d0da6ed2f89de9936ca3429110bc60a02f6e797665b8714d646fc25b45210ef2");
    std::reverse(txHash.begin(), txHash.end());
    auto redeemScript = Bitcoin::Script::lockScriptForAddress(myAddress, TWCoinTypeZcash).bytes;
    ...
    *input.mutable_plan() = plan.proto();
    ...
    Data serialized;
    signedTx.encode(serialized);
    ASSERT_EQ(hex(serialized), "");
}
