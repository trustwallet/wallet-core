...
const auto gMnemonic1 = "ripple scissors kick mammal hire column oak again sun offer wealth tomorrow wagon turn fatal";
...
TEST(HDWallet, createFromMnemonic) {
    {
        HDWallet wallet = HDWallet(gMnemonic1, gPassphrase);
        EXPECT_EQ(wallet.getMnemonic(), gMnemonic1);
        EXPECT_EQ(wallet.getPassphrase(), gPassphrase);
    ...
    {   // empty passphrase
        HDWallet wallet = HDWallet(gMnemonic1, "");
        EXPECT_EQ(wallet.getMnemonic(), gMnemonic1);
...
TEST(HDWallet, recreateFromEntropy) {
    {
        HDWallet wallet1 = HDWallet(gMnemonic1, gPassphrase);
        EXPECT_EQ(wallet1.getMnemonic(), gMnemonic1);
...
TEST(HDWallet, getExtendedPrivateKey) {
    const HDWallet wallet = HDWallet(gMnemonic1, "");
