...
TWData* _Nullable TWAESCTREncrypt(TWData* _Nonnull key, TWData* _Nonnull data,
    ...
    TWData* result = TWDataCreateWithSize(TWDataSize(data));
    aes_ctr_encrypt(TWDataBytes(data), TWDataBytes(result), TWDataSize(data), TWDataBytes(iv),
...
TWData* _Nullable TWAESCTRDecrypt(TWData* _Nonnull key, TWData* _Nonnull data,
    ...
    TWData* result = TWDataCreateWithSize(TWDataSize(data));
    aes_ctr_decrypt(TWDataBytes(data), TWDataBytes(result), TWDataSize(data), TWDataBytes(iv),
                    aes_ctr_cbuf_inc, &ctx);
