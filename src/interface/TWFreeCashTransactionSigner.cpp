...
struct TWFreeCashTransactionSigner *_Nonnull TWFreeCashTransactionSignerCreate(TW_FreeCash_Proto_SigningInput data) {
    Proto::SigningInput input;
    input.ParseFromArray(TWDataBytes(data), static_cast<int>(TWDataSize(data)));
    return new TWFreeCashTransactionSigner{ TransactionSigner<Transaction>(std::move(input)) };
}

struct TWFreeCashTransactionSigner *_Nonnull TWFreeCashTransactionSignerCreateWithPlan(TW_FreeCash_Proto_SigningInput data, TW_FreeCash_Proto_TransactionPlan planData) {
    Proto::SigningInput input;
    input.ParseFromArray(TWDataBytes(data), static_cast<int>(TWDataSize(data)));
    Proto::TransactionPlan plan;
    plan.ParseFromArray(TWDataBytes(planData), static_cast<int>(TWDataSize(planData)));
    return new TWFreeCashTransactionSigner{ TransactionSigner<Transaction>(std::move(input), std::move(plan)) };
}
