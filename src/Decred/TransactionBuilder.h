...
struct TransactionBuilder {
    ...
    static Bitcoin::TransactionPlan plan(const Bitcoin::Proto::SigningInput& input) {
        ...
        auto output_size = 2;
        auto calculator = Bitcoin::UnspentCalculator::getCalculator(static_cast<TWCoinType>(input.coin_type()));
    ...
    static Transaction build(const Bitcoin::TransactionPlan& plan, const std::string& toAddress, const std::string& changeAddress) {
        ...
        if (plan.change > 0) {
            ...
            tx.outputs.push_back(TransactionOutput(plan.change, /* version: */0, lockingScriptChange));
        }
