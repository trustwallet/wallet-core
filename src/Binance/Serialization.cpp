...
static inline std::string addressString(const std::string& bytes, bool testnet) {
    ...
    if (testnet) {
        return TAddress(data).string();
    }
    return Address(data).string();
}

static inline std::string validatorAddress(const std::string& bytes, bool testnet) {
    auto data = Data(bytes.begin(), bytes.end());
    if (testnet) {
        return Bech32Address(TAddress::hrpValidator, data).string();
    }
...
json signatureJSON(const Proto::SigningInput& input, bool testnet) {
    ...
    j["data"] = nullptr;
    j["memo"] = input.memo();
    j["msgs"] = json::array({orderJSON(input, testnet)});
...
json orderJSON(const Proto::SigningInput& input, bool testnet) {
    ...
    if (input.has_trade_order()) {
        ...
        j["sender"] = addressString(input.trade_order().sender(), testnet);
        ...
        j["sender"] = addressString(input.cancel_trade_order().sender(), testnet);
        j["symbol"] = input.cancel_trade_order().symbol();
    } else if (input.has_send_order()) {
        j["inputs"] = inputsJSON(input.send_order(), testnet);
        j["outputs"] = outputsJSON(input.send_order(), testnet);
    } else if (input.has_freeze_order()) {
        j["from"] = addressString(input.freeze_order().from(), testnet);
        j["symbol"] = input.freeze_order().symbol();
        j["amount"] = input.freeze_order().amount();
    } else if (input.has_unfreeze_order()) {
        j["from"] = addressString(input.unfreeze_order().from(), testnet);
        j["symbol"] = input.unfreeze_order().symbol();
        j["amount"] = input.unfreeze_order().amount();
    } else if (input.has_htlt_order()) {
        j["from"] = addressString(input.htlt_order().from(), testnet);
        j["to"] = addressString(input.htlt_order().to(), testnet);
        ...
        j["from"] = addressString(input.deposithtlt_order().from(), testnet);
        ...
        j["from"] = addressString(input.claimhtlt_order().from(), testnet);
        ...
        j["from"] = addressString(input.refundhtlt_order().from(), testnet);
        j["swap_id"] = hex(input.refundhtlt_order().swap_id());
    } else if (input.has_issue_order()) {
        j["from"] = addressString(input.issue_order().from(), testnet);
        ...
        j["from"] = addressString(input.mint_order().from(), testnet);
        j["amount"] = input.mint_order().amount();
        j["symbol"] = input.mint_order().symbol();
    } else if (input.has_burn_order()) {
        j["from"] = addressString(input.burn_order().from(), testnet);
        ...
        j["from"] = addressString(input.transfer_out_order().from(), testnet);
        j["to"] = addr.string();
        ...
        j["value"] = {
            {"delegator_addr", addressString(input.side_delegate_order().delegator_addr(), testnet)},
            {"validator_addr",validatorAddress(input.side_delegate_order().validator_addr(), testnet)},
        ...
        j["value"] = {
            {"delegator_addr", addressString(input.side_redelegate_order().delegator_addr(), testnet)},
            {"validator_src_addr", validatorAddress(input.side_redelegate_order().validator_src_addr(), testnet)},
            {"validator_dst_addr", validatorAddress(input.side_redelegate_order().validator_dst_addr(), testnet)},
        ...
        j["value"] = {
            {"delegator_addr", addressString(input.side_undelegate_order().delegator_addr(), testnet)},
            {"validator_addr", validatorAddress(input.side_undelegate_order().validator_addr(), testnet)},
        ...
        j["from"] = addressString(input.time_lock_order().from_address(), testnet);
        ...
        j["from"] = addressString(input.time_relock_order().from_address(), testnet);
        ...
        j["from"] = addressString(input.time_unlock_order().from_address(), testnet);
...
json inputsJSON(const Proto::SendOrder& order, bool testnet) {
    ...
    for (auto& input : order.inputs()) {
        j.push_back({
            {"address", addressString(input.address(), testnet)},
...
json outputsJSON(const Proto::SendOrder& order, bool testnet) {
    ...
    for (auto& output : order.outputs()) {
        j.push_back({
            {"address", addressString(output.address(), testnet)},
