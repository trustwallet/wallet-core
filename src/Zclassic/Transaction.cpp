...
Data Transaction::getSignatureHash(const Bitcoin::Script& scriptCode, size_t index,
    ...
    // Validate branchId for ZIP-205 compliance
    bool isValidBranch = (branchId == SaplingBranchID || 
                          branchId == BlossomBranchID || 
                          branchId == Nu6BranchID);
    assert(isValidBranch && "Invalid branchId for Zclassic transaction");
...
Bitcoin::Proto::Transaction Transaction::proto() const {
    ...
    // TODO: If needed to preserve Zclassic-specific fields (versionGroupId, expiryHeight,
    // valueBalance, branchId), consider using Zclassic::Proto::Transaction instead of
    // Bitcoin::Proto::Transaction. Currently, these fields are lost during proto conversion.
