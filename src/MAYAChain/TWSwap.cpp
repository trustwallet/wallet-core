...
#include <TrustWalletCore/TWMAYAChainSwap.h>

using namespace TW;

TWData* _Nonnull TWMAYAChainSwapBuildSwap(TWData* _Nonnull input) {
    MAYAChainSwap::Proto::SwapInput inputProto;
    MAYAChainSwap::Proto::SwapOutput outputProto;
    if (!inputProto.ParseFromArray(TWDataBytes(input), static_cast<int>(TWDataSize(input)))) {
        // error
        outputProto.mutable_error()->set_code(MAYAChainSwap::Proto::ErrorCode::Error_Input_proto_deserialization);
    ...
    auto&& [txInput, errorCode, error] = MAYAChainSwap::SwapBuilder::builder()
    ...
    if (errorCode != 0) {
        // error
        outputProto.mutable_error()->set_code(static_cast<MAYAChainSwap::Proto::ErrorCode>(errorCode));
        outputProto.mutable_error()->set_message(error);
    } else {
        // no error
        outputProto.mutable_error()->set_code(MAYAChainSwap::Proto::ErrorCode::OK);
        outputProto.mutable_error()->set_message("");

        switch (fromChain) {
        case MAYAChainSwap::Proto::BTC:
        case MAYAChainSwap::Proto::DOGE:
        case MAYAChainSwap::Proto::BCH:
        case MAYAChainSwap::Proto::LTC: {
        ...
        case MAYAChainSwap::Proto::ETH:
        case MAYAChainSwap::Proto::AVAX: {
        ...
        default:
            outputProto.mutable_error()->set_code(MAYAChainSwap::Proto::ErrorCode::Error_Unsupported_from_chain);
