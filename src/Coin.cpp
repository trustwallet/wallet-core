...
// TANGEM
void TW::anyCoinSignExternally(TWCoinType coinType, const Data& dataIn, Data& dataOut, std::function<Data(Data)> externalSigner) {
    auto* dispatcher = coinDispatcher(coinType);
    assert(dispatcher != nullptr);
    dispatcher->signExternally(coinType, dataIn, dataOut, externalSigner);
}
