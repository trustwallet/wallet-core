...
std::vector<uint8_t> Transaction::getPreImage(const Script& scriptCode, size_t index,
    ...
    reinterpret_cast<const TW::Peercoin::OutPoint&>(inputs[index].previousOutput).encode(data);
...
std::vector<uint8_t> Transaction::getPrevoutHash() const {
    ...
    for (auto& input : inputs) {
        auto& outpoint = reinterpret_cast<const TW::Peercoin::OutPoint&>(input.previousOutput);
...
void Transaction::serializeInput(size_t subindex, const Script& scriptCode, size_t index,
    ...
    reinterpret_cast<const TW::Peercoin::OutPoint&>(inputs[subindex].previousOutput).encode(data);
