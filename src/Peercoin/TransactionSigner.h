...
/// Helper class that performs Peercoin transaction signing.
template <typename Transaction>
class TransactionSigner {
  private:
    /// Private key and redeem script provider for signing.
    Peercoin::Proto::SigningInput input;
  ...
  public:
    ...
    TransactionSigner(Peercoin::Proto::SigningInput &&input)
    ...
    /// a hash type.
    TransactionSigner(Peercoin::Proto::SigningInput &&input, const TransactionPlan &plan)
...
/// Wrapper for Peercoin C interface.
struct TWPeercoinTransactionSigner {
    TW::Peercoin::TransactionSigner<TW::Peercoin::Transaction> impl;
};
