...
json Signer::buildTransactionJSON(const Data& signature) const {    
    auto sig = Cosmos_old::Proto::Signature();
    ...
    auto transaction = Cosmos_old::Proto::Transaction();
...
Data Signer::encodeTransaction(const Cosmos_old::Proto::Signature& sig) const {
    auto transaction = Cosmos_old::Proto::Transaction();
    *transaction.mutable_fee() = input.fee();
    transaction.set_memo(input.memo());
...
Cosmos_old::Proto::Signature Signer::encodeSignature(const PublicKey& publicKey, const Data& signature) const {
    auto sig = Cosmos_old::Proto::Signature();
    sig.set_public_key(publicKey.bytes.data(), publicKey.bytes.size());
    sig.set_signature(signature.data(), signature.size());
