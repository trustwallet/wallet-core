...
std::vector<uint8_t> Transaction::getPreImage(const Script& scriptCode, size_t index,
    ...
    reinterpret_cast<const TW::FreeCash::OutPoint&>(inputs[index].previousOutput).encode(data);
...
std::vector<uint8_t> Transaction::getPrevoutHash() const {
    ...
    for (auto& input : inputs) {
        auto& outpoint = reinterpret_cast<const TW::FreeCash::OutPoint&>(input.previousOutput);
...
std::vector<uint8_t> Transaction::getSignatureHash(const Script& scriptCode, size_t index,
                                                   enum TWFreeCashSigHashType hashType, uint64_t amount,
                                                   TWFreeCashSignatureVersion version) const {
    switch (version) {
    case BASE:
...
std::vector<uint8_t> Transaction::getSignatureHashWitnessV0(const Script& scriptCode, size_t index,
                                                            enum TWFreeCashSigHashType hashType,
...
std::vector<uint8_t> Transaction::getSignatureHashBase(const Script& scriptCode, size_t index,
    ...
    auto serializedInputCount =
        (hashType & TWFreeCashSigHashTypeAnyoneCanPay) != 0 ? 1 : inputs.size();
    ...
    auto hashNone = (hashType & 0x1f) == TWFreeCashSigHashTypeNone;
    auto hashSingle = (hashType & 0x1f) == TWFreeCashSigHashTypeSingle;
...
void Transaction::serializeInput(size_t subindex, const Script& scriptCode, size_t index,
                                 enum TWFreeCashSigHashType hashType, std::vector<uint8_t>& data) const {
    ...
    reinterpret_cast<const TW::FreeCash::OutPoint&>(inputs[subindex].previousOutput).encode(data);
