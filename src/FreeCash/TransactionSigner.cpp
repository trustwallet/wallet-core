...
Result<Transaction> TransactionSigner<Transaction>::sign() {
    ...
    const bool hashSingle =
        ((input.hash_type() & ~TWFreeCashSigHashTypeAnyoneCanPay) == TWFreeCashSigHashTypeSingle);
    for (auto i = 0; i < plan.utxos.size(); i += 1) {
        auto& utxo = plan.utxos[i];

        // Only sign TWFreeCashSigHashTypeSingle if there's a corresponding output
...
Result<void> TransactionSigner<Transaction>::sign(Script script, size_t index,
                                                  const FreeCash::Proto::UnspentTransaction& utxo) {
    Script redeemScript;
    std::vector<Data> results;
    std::vector<Data> witnessStack;

    uint32_t signatureVersion = [this]() {
        if ((input.hash_type() & TWFreeCashSigHashTypeFork) != 0) {
            return WITNESS_V0;
        } else {
            return BASE;
        }
...
Result<std::vector<Data>> TransactionSigner<Transaction>::signStep(
    Script script, size_t index, const FreeCash::Proto::UnspentTransaction& utxo, uint32_t version) {
...
template class TW::FreeCash::TransactionSigner<FreeCash::Transaction>;
