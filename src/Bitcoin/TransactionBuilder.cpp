...
TransactionPlan TransactionBuilder::plan(const Bitcoin::Proto::SigningInput& input) {
    ...
    if (input.amount() == 0 && !maxAmount) {
        plan.error = Error(Common::Proto::Error_zero_amount_requested, "Zero amount requested");
    } else if (input.utxo().empty()) {
        plan.error = Error(Common::Proto::Error_missing_input_utxos, "Missing input UTXOs");
    } else {
        // select UTXOs
        ...
        if (plan.utxos.size() == 0) {
            plan.amount = 0;
            plan.error = Error(Common::Proto::Error_not_enough_utxos, "Not enough non-dust input UTXOs");
        } else {
