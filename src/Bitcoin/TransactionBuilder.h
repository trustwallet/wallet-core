...
struct TransactionBuilder {
    ...
    static TransactionPlan plan(Bitcoin::Proto::SigningInput input) {
        ...
        auto output_size = 2;
        auto unspentSelector = UnspentSelector();
        if (input.use_max_amount() && UnspentSelector::sum(input.utxo()) == plan.amount) {
            ...
            for (auto utxo: input.utxo()) {
                if (utxo.amount() > unspentSelector.singleInputFeeCalculator(input.byte_fee())) {
                    input_size++;
                    newAmount += utxo.amount();
                }
            }

            plan.amount = newAmount - unspentSelector.feeCalculator(input_size, output_size, input.byte_fee());
        ...
        plan.fee = unspentSelector.feeCalculator(plan.utxos.size(), output_size, input.byte_fee());
