...
std::tuple<uint256_t, uint256_t, uint256_t> Harmony::Signer::values(const uint256_t &chainID,
...
Harmony::Signer::sign(const uint256_t &chainID, const PrivateKey &privateKey, const Data& hash) noexcept {
    auto signature = privateKey.sign(hash, TWCurveSECP256k1);
    return values(chainID, signature);
}

template <typename T>
Harmony::Proto::SigningOutput Harmony::Signer::prepareOutput(const Data& encoded, const T &transaction) noexcept {
    auto protoOutput = Harmony::Proto::SigningOutput();
...
Harmony::Proto::SigningOutput Harmony::Signer::sign(const Harmony::Proto::SigningInput &input) noexcept {
...
std::string Harmony::Signer::signJSON(const std::string& json, const Data& key) {
...
Harmony::Proto::SigningOutput Harmony::Signer::signTransaction(const Proto::SigningInput &input) noexcept {
...
Harmony::Proto::SigningOutput Harmony::Signer::signCreateValidator(const Harmony::Proto::SigningInput &input) noexcept {
    auto key = PrivateKey(Data(input.private_key().begin(), input.private_key().end()));
...
Harmony::Proto::SigningOutput Harmony::Signer::signEditValidator(const Proto::SigningInput &input) noexcept {
    auto key = PrivateKey(Data(input.private_key().begin(), input.private_key().end()));
...
Harmony::Proto::SigningOutput Harmony::Signer::signDelegate(const Proto::SigningInput &input) noexcept {
    auto key = PrivateKey(Data(input.private_key().begin(), input.private_key().end()));
...
Harmony::Proto::SigningOutput Harmony::Signer::signUndelegate(const Proto::SigningInput &input) noexcept {
    auto key = PrivateKey(Data(input.private_key().begin(), input.private_key().end()));
...
Harmony::Proto::SigningOutput Harmony::Signer::signCollectRewards(const Proto::SigningInput &input) noexcept {
    auto key = PrivateKey(Data(input.private_key().begin(), input.private_key().end()));
...
template <typename T>
void Harmony::Signer::sign(const PrivateKey &privateKey, const Data& hash, T &transaction) const noexcept {
...
Data Harmony::Signer::rlpNoHash(const Transaction &transaction, const bool include_vrs) const noexcept {
    auto encoded = Data();
...
Data Harmony::Signer::rlpNoHash(const Staking<Directive> &transaction, const bool include_vrs) const
    noexcept {
    auto encoded = Data();
    using namespace TW::Ethereum;
...
Data Harmony::Signer::rlpNoHashDirective(const Staking<CreateValidator> &transaction) const noexcept {
    auto encoded = Data();
    using namespace TW::Ethereum;
...
Data Harmony::Signer::rlpNoHashDirective(const Staking<EditValidator> &transaction) const noexcept {
    auto encoded = Data();
    using namespace TW::Ethereum;
...
Data Harmony::Signer::rlpNoHashDirective(const Staking<Delegate> &transaction) const noexcept {
    auto encoded = Data();
    using namespace TW::Ethereum;
...
Data Harmony::Signer::rlpNoHashDirective(const Staking<Undelegate> &transaction) const noexcept {
    auto encoded = Data();
    using namespace TW::Ethereum;
    append(encoded, RLP::encode(transaction.stakeMsg.delegatorAddress.getKeyHash()));
    append(encoded, RLP::encode(transaction.stakeMsg.validatorAddress.getKeyHash()));
...
Data Harmony::Signer::rlpNoHashDirective(const Staking<CollectRewards> &transaction) const noexcept {
    auto encoded = Data();
    using namespace TW::Ethereum;
    append(encoded, RLP::encode(transaction.stakeMsg.delegatorAddress.getKeyHash()));
    return RLP::encodeList(encoded);
...
std::string Harmony::Signer::txnAsRLPHex(Transaction &transaction) const noexcept {
    return TW::hex(rlpNoHash(transaction, false));
}

template <typename Directive>
std::string Harmony::Signer::txnAsRLPHex(Staking<Directive> &transaction) const noexcept {
    return TW::hex(rlpNoHash<Directive>(transaction, false));
}

Data Harmony::Signer::hash(const Transaction &transaction) const noexcept {
    return Hash::keccak256(rlpNoHash(transaction, false));
}

template <typename Directive>
Data Harmony::Signer::hash(const Staking<Directive> &transaction) const noexcept {
