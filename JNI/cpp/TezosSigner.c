...
#include "TWJNI.h"
#include "TezosSigner.h"

jobject JNICALL Java_wallet_core_jni_TezosSigner_sign(JNIEnv *env, jclass thisClass, jobject input) {
    ...
    jbyteArray resultData = TWDataJByteArray(TWTezosSignerSign(inputData), env);
    jclass resultClass = (*env)->FindClass(env, "wallet/core/jni/proto/Tezos$SigningOutput");
    jmethodID parseFromMethodID = (*env)->GetStaticMethodID(env, resultClass, "parseFrom", "([B)Lwallet/core/jni/proto/Tezos$SigningOutput;");
