...
#include "TWJNI.h"
#include "AionSigner.h"

jobject JNICALL Java_wallet_core_jni_AionSigner_sign(JNIEnv *env, jclass thisClass, jobject input) {
    ...
    jbyteArray resultData = TWDataJByteArray(TWAionSignerSign(inputData), env);
    jclass resultClass = (*env)->FindClass(env, "wallet/core/jni/proto/Aion$SigningOutput");
    jmethodID parseFromMethodID = (*env)->GetStaticMethodID(env, resultClass, "parseFrom", "([B)Lwallet/core/jni/proto/Aion$SigningOutput;");
