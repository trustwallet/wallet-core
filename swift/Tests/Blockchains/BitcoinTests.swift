...
class BitcoinTransactionSignerTests: XCTestCase {
    ...
    func testSignNftInscriptionReveal() throws {
        ...
        let publicKey = privateKey.getPublicKeySecp256k1(compressed: false)
        let pubKeyHash = publicKey.bitcoinKeyHash
        let inputInscribe = BitcoinScript.buildNftInscription(mimeType: OrdMimeType.imagePng, payload: nftInscriptionImageData, pubkey: publicKey.data)
        let p2wpkh = BitcoinScript.buildPayToWitnessPubkeyHash(hash: pubKeyHash)
...
let nftInscriptionImageData = Data("""
...
let nftInscriptionRawHex = Data(hexString: """
