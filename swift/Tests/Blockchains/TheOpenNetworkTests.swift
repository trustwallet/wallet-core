...
class TheOpenNetworkTests: XCTestCase {
    func testAddressFromPrivateKey() {
        ...
        let address = AnyAddress(publicKey: publicKey, coin: .theOpenNetwork)
    ...
    func testAddressFromRawString() {
        ...
        let address = AnyAddress(string: addressString, coin: .theOpenNetwork)
        XCTAssertEqual(address!.description, "EQBm--PFwDv1yCeS-QTJ-L8oiUpqo9IT1BwgVptlSq3ts90Q")
    }
    ...
    func testSign() {
        ...
        let output: TheOpenNetworkSigningOutput = AnySigner.sign(input: input, coin: .theOpenNetwork)
