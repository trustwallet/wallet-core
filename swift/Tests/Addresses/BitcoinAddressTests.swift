...
class BitcoinAddressTests: XCTestCase {
    ...
    func testFromPrivateKey() {
        ...
        let address = BitcoinAddress.compatibleAddress(publicKey: publicKey, prefix: CoinTypeConfiguration.getP2SHPrefix(type: .bitcoin))
    ...
    func testFromPrivateKeySegwitAddress() {
        ...
        let address = BitcoinAddress(publicKey: publicKey, prefix: CoinTypeConfiguration.getP2PKHPrefix(type: .bitcoin))!
    ...
    func testValidDigiByteAddress() {
        ...
        XCTAssertEqual(CoinTypeConfiguration.getP2PKHPrefix(type: .digiByte), BitcoinAddress(string: addressString)?.prefix)
        ...
        XCTAssertEqual(CoinTypeConfiguration.getP2SHPrefix(type: .digiByte), BitcoinAddress(string: addressString2)?.prefix)
    ...
    func testValidRavencoinAddress() {
        ...
        XCTAssertEqual(CoinTypeConfiguration.getP2PKHPrefix(type: .ravencoin), BitcoinAddress(string: addressString)?.prefix)
        XCTAssertTrue(BitcoinAddress.isValidString(string: addressString),
                      "'\(addressString)' should be a valid Ravencoin address")

        let addressString2 = "rPWwn5h4QFZNaz1XmY39rc73sdYGGDdmq1"

        XCTAssertEqual(CoinTypeConfiguration.getP2SHPrefix(type: .ravencoin), BitcoinAddress(string: addressString2)?.prefix)
        XCTAssertTrue(BitcoinAddress.isValidString(string: addressString2),
