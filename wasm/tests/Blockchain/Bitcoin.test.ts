...
describe("Bitcoin", () => {
  ...
  it("test Bitcoin sign arbitrary message, validate (recover)", () => {
    ...
    const address = "bc1qten42eesehw0ktddcp0fws7d3ycsqez3f7d5yt";
    const addressDerived = AnyAddress.createWithPublicKey(publicKey, coin);
    assert.equal(addressDerived.description(), address);
    ...
    const addressRecovered = AnyAddress.createWithPublicKey(publicKeyRecovered, coin);
    assert.equal(addressRecovered.description(), address);
  });
});
